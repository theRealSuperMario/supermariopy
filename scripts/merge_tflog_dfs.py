...
def main(files2stack: str, range_list: str, out_name: str):
    """Merges multiple dataframes generated with tflogs2pandas.py into one single
    dataframe. This useful when a training is continued from a certain checkpoint.
    Then one can easily stack them together. It is also possible to filter the dataframe
    on the "step" column to only include a certain range. Dataframe files (currently
    only csv files are supported) must be provided as comma-seperated list.

    # stack df1.csv and df2.csv together into df_final.csv
    merge_tflog_dfs.py df1.csv,df2.csv -o df_final.csv

    # stack df1.csv and df2.csv together into df_final.csv. Take only steps within
    # [0, 3000] from df1.csv
