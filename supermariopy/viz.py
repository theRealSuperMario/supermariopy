...
def show_active_parts(
    ...
    ----------
    parts : np.ndarray
        tensor of binary masks indicating mutually exclusive parts or segments of a
        segmentation.
        Shaped [N, H, W, P]
    active_idx : np.array or list
        list of indices indicating which part (indexing into last dimension of parts)
        are active.
    ...
    background_color : np.ndarray, default np.array([1.0, 1.0, 1.0])
        Color value in range [0, 1] for background. Note that this only makes sense if
        parts does not provide a background itself.
        If part contains a background part, it will be assigned a background color
        specified by the color map.
    """
